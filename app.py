...
def load_web_data():
    ...
    content = blob.download_as_string()         # Download bytes
    return pd.read_csv(io.BytesIO(content))    # Read into pandas

app = Flask(__name__)

# Load once (or you can reload inside each request if you need fresh data)
df = load_web_data()
