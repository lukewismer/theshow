...
name_map = {
    "CON R": "contact_right_new",
    "CON L": "contact_left_new",
    "POW R": "power_right_new",
    "POW L": "power_left_new",
    "VIS": "plate_vision_new",
    "DISC": "plate_discipline_new",
    "CLT": "batting_clutch_new",
    "K/9": "k_per_bf_new",
    "BB/9": "bb_per_bf_new",
    "HR/9": "hr_per_bf_new",
    "PCLT": "pitching_clutch_new",
    "STA": "stamina_new"
}
...
def merge_updates(input_csv, year, cache_dir='.'):
    ...
    for label, uid, date in UPDATE_IDS[year]:
        ...
        for change in resp.json().get("attribute_changes", []):
            ...
            stat_changes = change.get("changes", [])
            for c in stat_changes:
                abbr = c["name"]
                col_new = name_map.get(abbr)
                if not col_new:
                    continue
                old_val = int(base.get(col_new.rsplit("_",1)[0], 0))
                delta   = int(c["delta"])
                base[col_new] = old_val + delta

            # ensure every _new column exists (zero‐delta if missing)
            for col_new in name_map.values():
                if col_new not in base:
                    base[col_new] = int(base.get(col_new.rsplit("_",1)[0], 0))
